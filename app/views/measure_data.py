...
import pytz
from app.models import MeasurementData ,InterlinkData # Import your model
...
def delete_measure_data(request):
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            comp_sr_no = data.get('input_value', '')
            part_model = data.get('part_model', '')
            password = data.get('password', '')

            # Check MeasurementData
            existing_record = MeasurementData.objects.filter(comp_sr_no=comp_sr_no, part_model=part_model)
            if existing_record.exists():
                if not password:
                    return JsonResponse({
                        'status': 'exists',
                        'message': f'Punch number "{comp_sr_no}" already exists for model "{part_model}". Overwrite?'
                    })

                if password != 'admin':
                    return JsonResponse({'success': False, 'message': 'Invalid credentials!'})

                # Delete from MeasurementData (SQLite)
                deleted_count, _ = existing_record.delete()

                # Delete from InterlinkData in BOTH SQLite and MSSQL
                interlink_records = InterlinkData.objects.filter(PartModel=part_model, CompSrNo=comp_sr_no)

                # Delete in SQLite
                interlink_records.using('default').delete()

                # Delete in MSSQL
                interlink_records.using('mssql').delete()

                return JsonResponse({'success': True, 'message': 'Deleted from both SQLite and MSSQL!'})

            else:
                return JsonResponse({'success': True, 'message': 'No existing record to delete.'})
